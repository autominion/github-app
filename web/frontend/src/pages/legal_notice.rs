...
pub fn LegalNoticePage() -> impl IntoView {
    let markdown_content = LocalResource::new(|| async move {
        ...
        let url = Url::parse(&href)
            ...
            .join("/static/legal-notice.md")
            .expect("Failed to construct legal notice URL");
        let response = reqwest::get(url).await;
        match response {
            Ok(res) => {
                if res.status().is_success() {
                    res.text().await.unwrap_or_else(|_| "Failed to load legal notice.".to_string())
                } else {
                    "Failed to load legal notice.".to_string()
                }
            }
            Err(_) => "Failed to load legal notice.".to_string(),
        }
    });
